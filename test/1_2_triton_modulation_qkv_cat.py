...
"""
13320192 = (24 * 4336 * 128)
3072 : num_heads * head_dim = hidden_dim
9216 : 3072 * 3 (qkv)
2359296 : 
9216*x2
2359296*x3
...
@triton.jit
def triton_qkv_concat(in_ptr0, in_ptr1, out_ptr0, out_ptr1, out_ptr2, seq_len=4336, num_heads=24, head_dim=128, XBLOCK : tl.constexpr):
    xoffset = tl.program_id(0) * XBLOCK
    xindex = xoffset + tl.arange(0, XBLOCK)[:]

    hidden_dim = num_heads * head_dim # 3072 = 24 * 128
    qkv_block_dim = hidden_dim * 3    # 9216

    qkv_block_idx = (xindex // hidden_dim) % seq_len
    batch_idx = (xindex // hidden_dim*seq_len)
    qkv_idx = xindex % hidden_dim          # idx_
    head_idx = xindex % head_dim           # idx_head_dim
    seq_idx = (xindex // head_dim) % num_heads
    tmp0 = qkv_block_idx
    ...
    ##############################################################################################################
    #key
    tmp5 = tl.load(in_ptr0 + (hidden_dim * 0 + qkv_idx + (qkv_block_idx * qkv_block_dim) + (num_heads*head_dim*batch_idx)), tmp4, other=0.0).to(tl.float32)
    tmp6 = tl.full(tmp5.shape, 0.0, tmp5.dtype)
    tmp7 = tl.where(tmp4, tmp5, tmp6)
    tmp8 = tmp0 >= tmp3
    
    tmp11 = tl.load(in_ptr1 + ((-num_heads*head_dim + (hidden_dim * 0)) + qkv_idx + (qkv_block_idx * qkv_block_dim) + (hidden_dim*seq_len*batch_idx)), tmp8, other=0.0).to(tl.float32)
    tmp12 = tl.full(tmp11.shape, 0.0, tmp11.dtype)
    tmp13 = tl.where(tmp8, tmp11, tmp12)
    tmp14 = tl.where(tmp4, tmp7, tmp13)
    tl.store(out_ptr0 + (head_idx + (head_dim*qkv_block_idx) + (head_dim*seq_len*seq_idx) + (hidden_dim*seq_len*batch_idx)), tmp14, None)

    ##############################################################################################################
    ##############################################################################################################
    #query
    tmp5 = tl.load(in_ptr0 + (hidden_dim * 1 + qkv_idx + (qkv_block_idx * qkv_block_dim) + (num_heads*head_dim*batch_idx)), tmp4, other=0.0).to(tl.float32)
    tmp6 = tl.full(tmp5.shape, 0.0, tmp5.dtype)
    tmp7 = tl.where(tmp4, tmp5, tmp6)
   
    tmp11 = tl.load(in_ptr1 + ((-num_heads*head_dim + (hidden_dim * 1)) + qkv_idx + (qkv_block_idx * qkv_block_dim) + (hidden_dim*seq_len*batch_idx)), tmp8, other=0.0).to(tl.float32)
    tmp12 = tl.full(tmp11.shape, 0.0, tmp11.dtype)
    tmp13 = tl.where(tmp8, tmp11, tmp12)
    tmp14 = tl.where(tmp4, tmp7, tmp13)
    tl.store(out_ptr1 + (head_idx + (head_dim*qkv_block_idx) + (head_dim*seq_len*seq_idx) + (hidden_dim*seq_len*batch_idx)), tmp14, None)

    ##############################################################################################################
    ##############################################################################################################
    #value
    tmp5 = tl.load(in_ptr0 + (hidden_dim * 2 + qkv_idx + (qkv_block_idx * qkv_block_dim) + (num_heads*head_dim*batch_idx)), tmp4, other=0.0).to(tl.float32)
    tmp6 = tl.full(tmp5.shape, 0.0, tmp5.dtype)
    tmp7 = tl.where(tmp4, tmp5, tmp6)
   
    tmp11 = tl.load(in_ptr1 + ((-num_heads*head_dim + (hidden_dim * 2)) + qkv_idx + (qkv_block_idx * qkv_block_dim) + (hidden_dim*seq_len*batch_idx)), tmp8, other=0.0).to(tl.float32)
    tmp12 = tl.full(tmp11.shape, 0.0, tmp11.dtype)
    tmp13 = tl.where(tmp8, tmp11, tmp12)
    tmp14 = tl.where(tmp4, tmp7, tmp13)
    tl.store(out_ptr2 + (head_idx + (head_dim*qkv_block_idx) + (head_dim*seq_len*seq_idx) + (hidden_dim*seq_len*batch_idx)), tmp14, None)
...
def qkv_concat(img_qkv, txt_qkv, num_heads):
    ...
    triton_qkv_concat[grid](txt_qkv, img_qkv, output_q, output_k, output_v, seq_len=4336, num_heads=24, head_dim=128, XBLOCK=1024)
